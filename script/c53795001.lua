...
function cm.filter(c,ec,tp)
	return c:IsSetCard(0xc534) and c:IsType(TYPE_EQUIP) and c:CheckEquipTarget(ec) and not Duel.IsExistingMatchingCard(function(c,code)return c:IsFaceup() and c:IsCode(code)end,tp,LOCATION_ONFIELD,0,1,nil,c:GetCode())
end
function cm.op(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	if c:IsRelateToEffect(e) and Duel.SelectYesNo(tp,aux.Stringid(m,1)) then
		Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_ATTRIBUTE)
		local att=Duel.AnnounceAttribute(tp,1,0x3f&(~c:GetAttribute()))
		local e1=Effect.CreateEffect(c)
		e1:SetType(EFFECT_TYPE_SINGLE)
