...
function cm.initial_effect(c)
	...
	e6:SetValue(c22348368.val)
	c:RegisterEffect(e6)
	--
	
end
function c22348368.sstfilter(c)
	return c:IsFaceup() and c:IsRace(RACE_ZOMBIE)
end
function c22348368.val(e,c)
	return Duel.GetMatchingGroupCount(c22348368.sstfilter,c:GetControler(),LOCATION_MZONE,0,nil)*800
end
...
function c22348368.sptgd(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.GetLocationCount(tp,LOCATION_MZONE)>0
		and Duel.IsExistingMatchingCard(c22348368.spfilter,tp,LOCATION_GRAVE,0,1,nil,e,tp) end
	Duel.SetOperationInfo(0,CATEGORY_SPECIAL_SUMMON,nil,1,tp,LOCATION_GRAVE)
end
function c22348368.spopd(e,tp,eg,ep,ev,re,r,rp)
	if Duel.GetLocationCount(tp,LOCATION_MZONE)<=0 then return end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_SPSUMMON)
	local g=Duel.SelectMatchingCard(tp,c22348368.spfilter,tp,LOCATION_GRAVE,0,1,1,nil,e,tp)
